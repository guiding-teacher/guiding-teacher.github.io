...
let examsTableBody, studentsTableGlobalBody, examLogTableBody, successLogTableBody, failureLogTableBody,
    ...
    resultsDistributionChartCtx, questionsTableBody, adminEmailDisplay,
    sidebarAdminName, sidebarAdminEmail, settingsAdminName, settingsAdminEmail, settingsAdminPhone,
    settingsAdminInstitution, settingsAdminGovernorate, settingsAdminDob, settingsAdminCreated,
    editProfileBtn, editProfileForm, profileDisplay, adminNameInput, adminPhoneInput,
    adminInstitutionInput, adminGovernorateInput, adminDobInput, updateProfileBtn,
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            try {
                ...
                alert("حدث خطأ في تحميل بيانات المشرف");
            }
        } else {
...
async function loadAdminData(adminId) {
    try {
        ...
        if (adminDoc.exists()) {
            ...
            console.warn("No admin data found for:", adminId);
            currentAdminData = { name: "مشرف", email: auth.currentUser?.email || "غير معروف" };
        }
    } catch (error) {
        console.error("Error loading admin data:", error);
        currentAdminData = { name: "مشرف", email: auth.currentUser?.email || "غير معروف" };
...
function updateAdminInfoDisplay() {
    if (currentAdminData) {
        if (sidebarAdminName) sidebarAdminName.textContent = currentAdminData.name;
        if (sidebarAdminEmail) sidebarAdminEmail.textContent = currentAdminData.email;
        if (settingsAdminName) settingsAdminName.textContent = currentAdminData.name;
        if (settingsAdminEmail) settingsAdminEmail.textContent = currentAdminData.email;
        if (settingsAdminPhone) settingsAdminPhone.textContent = currentAdminData.phone || "غير محدد";
        if (settingsAdminInstitution) settingsAdminInstitution.textContent = currentAdminData.institution || "غير محدد";
        if (settingsAdminGovernorate) settingsAdminGovernorate.textContent = currentAdminData.governorate || "غير محدد";
        if (settingsAdminDob) settingsAdminDob.textContent = currentAdminData.dob || "غير محدد";
        if (settingsAdminCreated) {
            const createdDate = currentAdminData.createdAt?.toDate?.() || new Date();
            settingsAdminCreated.textContent = createdDate.toLocaleDateString('ar-EG');
        }
    }
}
...
async function handleLogout() {
    try {
        if (currentAdminId) {
            await updateDoc(doc(db, "admins", currentAdminId), { lastLogout: new Date() });
        }
        await signOut(auth);
...
// The following sections are kept for completeness but have no logic changes from the original file.
// =================================================================
// --- Question Management ---
// =================================================================
...
async function loadQuestions(examId) {
    ...
    try {
        const exam = allExamsCache[examId];
        const questions = exam ? exam.questions : null;
        ...
        for (const qId of sortedQuestionIds) {
            ...
            const correctOptionText = question.options[question.correctOptionIndex] || 'غير محدد';
            const row = questionsTableBody.insertRow();
            row.innerHTML = `<td>${question.text}</td><td>${question.points || 1}</td><td>${correctOptionText}</td><td>${question.isMandatory ? 'نعم' : 'لا'}</td><td><button class="btn-edit" onclick="editQuestion('${examId}', '${qId}')">تعديل</button><button class="btn-delete" onclick="deleteQuestion('${examId}', '${qId}')">حذف</button></td>`;
        }
    } catch (error) {
...
async function saveQuestion() {
    ...
    const options = Array.from(questionOptionsContainer.querySelectorAll('.option-group input[type="text"]')).map(input => input.value.trim());
    const correctOptionRadio = questionOptionsContainer.querySelector('input[name="correct_option"]:checked');
    if (!examId || !questionText || options.length < 2 || options.some(opt => !opt) || !correctOptionRadio || isNaN(points) || points <= 0) {
        alert('يرجى إدخال نص السؤال، درجة صحيحة، خيارين على الأقل، وتحديد الإجابة الصحيحة.');
        return;
    }
    const correctOptionIndex = parseInt(correctOptionRadio.value);
    const questionData = { text: questionText, points, options, correctOptionIndex, isMandatory };
    try {
        const newQuestionId = qId || 'q' + Date.now();
        await updateDoc(doc(db, "exams", examId), { [`questions.${newQuestionId}`]: questionData });
        alert(`تم ${qId ? 'تحديث' : 'حفظ'} السؤال بنجاح!`);
        allExamsCache[examId].questions[newQuestionId] = questionData;
        addEditQuestionForm.classList.add('hidden');
...
async function editQuestion(examId, qId) {
    const question = allExamsCache[examId]?.questions?.[qId];
    if (question) {
        addEditQuestionForm.classList.remove('hidden');
        ...
        questionIsMandatoryCheckbox.checked = question.isMandatory === true;
        questionOptionsContainer.innerHTML = '';
        questionOptionCount = 0;
        question.options.forEach((opt, index) => addOptionInput(opt, index === question.correctOptionIndex));
    }
}
...
async function deleteQuestion(examId, qId) {
    if (confirm('هل أنت متأكد من حذف هذا السؤال؟')) {
        try {
            await updateDoc(doc(db, "exams", examId), { [`questions.${qId}`]: deleteField() });
            delete allExamsCache[examId].questions[qId];
...
async function showParticipantManagement(examId, examTitle) {
    ...
    try {
        ...
        const assignedStudentIds = new Set(participantsSnapshot.docs.map(doc => doc.id));
        const availableStudents = allStudentsCache.filter(student => !assignedStudentIds.has(student.id));
        assignStudentSelect.innerHTML = availableStudents.length === 0 ? '<option value="">-- كل الطلاب معينون لهذا الاختبار --</option>' : '<option value="">-- اختر طالباً --</option>' + availableStudents.map(student => `<option value="${student.id}">${student.data.name}</option>`).join('');
    } catch (e) {
        console.error("Error populating dropdown:", e);
        assignStudentSelect.innerHTML = '<option value="">-- خطأ في تحميل الطلاب --</option>';
    }
...
async function assignStudentToExam() {
    ...
    try {
        ...
        await setDoc(participantRef, { name: selectedStudent.data.name, status: 'not_started' });
        ...
        assignStudentFeedback.textContent = 'حدث خطأ أثناء تعيين الطالب.';
    }
}
...
async function assignAllUnassignedStudents() {
    const examId = currentManagingParticipantsExamIdInput.value;
    if (!examId) return;
    if (!confirm("هل أنت متأكد من تعيين كل الطلاب غير المعينين حاليًا لهذا الاختبار؟")) return;
    assignStudentFeedback.textContent = "جاري تعيين كل الطلاب...";
    try {
        const participantsSnapshot = await getDocs(collection(db, "exams", examId, "participants"));
        const assignedStudentIds = new Set(participantsSnapshot.docs.map(doc => doc.id));
        const availableStudents = allStudentsCache.filter(student => !assignedStudentIds.has(student.id));
        if (availableStudents.length === 0) {
            assignStudentFeedback.textContent = "لا يوجد طلاب غير معينين لتعيينهم.";
            return;
        }
        const promises = availableStudents.map(student => {
            const participantRef = doc(db, "exams", examId, "participants", student.id);
            return setDoc(participantRef, { name: student.data.name, status: 'not_started' });
        });
        await Promise.all(promises);
        assignStudentFeedback.textContent = `تم تعيين ${availableStudents.length} طالب بنجاح!`;
        showParticipantManagement(examId, selectedExamTitleParticipantsSpan.textContent);
    } catch (error) {
        console.error("Error assigning all students:", error);
        assignStudentFeedback.textContent = "حدث خطأ أثناء التعيين الجماعي.";
    }
}
...
async function loadParticipants(examId) {
    ...
    try {
        ...
        if (participantsSnapshot.empty) {
            participantsTableBody.innerHTML = '<tr><td colspan="5">لم يتم تعيين أي طلاب لهذا الاختبار بعد.</td></tr>';
            return;
        }
        participantsTableBody.innerHTML = '';
        participantsSnapshot.forEach(pDoc => {
            const studentId = pDoc.id;
            const studentData = allStudentsCache.find(s => s.id === studentId)?.data;
            if (!studentData) return;
            const participantData = pDoc.data();
            const scoreText = (participantData.score !== undefined) ? `${participantData.score} / ${participantData.totalPossiblePoints}` : '---';
            const row = participantsTableBody.insertRow();
            row.innerHTML = `<td>${studentData.name}</td><td>${studentData.accessCode}</td><td>${translateStatus(participantData.status)}</td><td>${scoreText}</td><td><button class="btn-delete" onclick="unassignStudent('${studentId}', '${examId}')">إلغاء التعيين</button></td>`;
        });
    } catch (error) {
...
async function unassignStudent(studentId, examId) {
    if (confirm('هل أنت متأكد من إلغاء تعيين هذا الطالب من الاختبار؟ سيتم حذف بيانات تقدمه في الاختبار أيضاً.')) {
        try {
            const participantRef = doc(db, "exams", examId, "participants", studentId);
            ...
            console.error("Error un-assigning student:", error);
...
async function calculateAndDisplayStats(examId) {
    try {
        ...
        const participantsSnapshot = await getDocs(collection(db, "exams", examId, "participants"));
        if (participantsSnapshot.empty) {
            statsTotalParticipants.textContent = '0';
            statsAvgPercentage.textContent = '0%';
            statsPassedCount.textContent = '0';
            ...
            if (data.status === 'finished' || data.status === 'timed_out') {
                ...
                if (data.answers) {
                    Object.keys(data.answers).forEach(qId => {
                        ...
                        if (question && data.answers[qId] === question.correctOptionIndex) {
                            questionStats[qId].corrects++;
                        }
                    });
                }
            }
        ...
        displayFailureLog(failedParticipants);
    } catch (error) {
...
function populateParticipantRow(row, data) {
    ...
    row.innerHTML = `<td>${data.name || '---'}</td><td>${data.score} / ${data.totalPossiblePoints}</td><td>${data.percentage}%</td><td>${startTime}</td><td>${endTime}</td><td>${duration}</td>`;
}
...
function displayResultsChart(participantsData, passingPercentage) {
    ...
    resultsChart = new Chart(resultsDistributionChartCtx, {
        type: 'pie',
        data: {
            labels: [`ناجح (>= ${passingPercentage}%)`, `راسب (< ${passingPercentage}%)`],
            datasets: [{
                data: [passed, failed],
...
function displayQuestionDifficulty(questionStats, examQuestions) {
    ...
    const sortedQuestionIds = Object.keys(examQuestions).sort();
    for (const qId of sortedQuestionIds) {
        const stats = questionStats[qId] || { attempts: 0, corrects: 0 };
        const correctPercentage = stats.attempts > 0 ? ((stats.corrects / stats.attempts) * 100).toFixed(1) : '0.0';
        const row = questionStatsTableBody.insertRow();
        row.innerHTML = `<td>${examQuestions[qId].text}</td><td style="background-color: ${correctPercentage > 70 ? '#d4edda' : (correctPercentage < 40 ? '#f8d7da' : '#fff3cd')}">${correctPercentage}% (${stats.corrects}/${stats.attempts} مجيبين)</td>`;
    }
}
async function exportFullLogToExcel() {
    ...
    const q = query(collection(db, "exam_log"), where("adminId", "==", currentAdminId), orderBy("endTime", "desc"));
    const snapshot = await getDocs(q);
    if (snapshot.empty) return alert('لا توجد سجلات لتصديرها.');
    const excelData = snapshot.docs.map(doc => {
        const log = doc.data();
        return {
            "تاريخ الإنهاء": log.endTime.toDate().toLocaleString('ar-EG'),
            "اسم الطالب": log.studentName,
            "عنوان الاختبار": log.examTitle,
            "الدرجة": log.score,
            "الدرجة الكلية": log.totalPossiblePoints,
            "النسبة": log.percentage,
            "الحالة": log.statusText,
        };
    });
    const worksheet = XLSX.utils.json_to_sheet(excelData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "السجل الكامل");
    XLSX.writeFile(workbook, "FullExamLog.xlsx");
}
...
async function deleteAllLogs() {
    if (!currentAdminId) return;
    const confirmation = prompt("للتأكيد، اكتب 'حذف' في المربع أدناه. هذه العملية لا يمكن التراجع عنها.");
    if (confirmation !== 'حذف') {
        alert("لم يتم الحذف. الكتابة غير مطابقة.");
        return;
    }
    if (!confirm("تأكيد نهائي: هل أنت متأكد من حذف جميع سجلات الاختبارات بشكل دائم؟")) return;
    try {
        const q = query(collection(db, "exam_log"), where("adminId", "==", currentAdminId));
        ...
        await batch.commit();
        alert(`تم حذف ${logSnapshot.size} سجل بنجاح.`);
        loadExamLog();
    } catch (error) {
...
async function updateAdminProfile() {
    ...
    if (!name || !phone || !institution || !governorate || !dob) {
    ...
    try {
        const dataToUpdate = {
            name, phone, institution, governorate, dob,
            updatedAt: new Date()
        };
