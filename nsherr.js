...
// --- تعديل جوهري هنا ---
// تم تعديل الدالة لتضيف النص المساعد مع الرابط مباشرة
// في ملف nsherr.js
...
function linkify(htmlContent) {
    ...
    return htmlContent.replace(urlRegex, (url) => {
        if (htmlContent.includes(`href="${url}"`)) { 
            return url; 
        }
        // إضافة class="auto-link" هنا
        return `<span class="link-helper-text">الدخول للخدمة:</span><a href="${url}" target="_blank" rel="noopener noreferrer" class="auto-link">${url}</a>`;
    });
}

document.addEventListener("DOMContentLoaded", () => {
  const olLists = document.querySelectorAll("ol");
  olLists.forEach(ol => ol.classList.add("spacing"));
});
...
function displayVideoPost(post, postId, index) {
    ...
    const cleanedDescription = removeExtraLineBreaks(post.description || '');
    const processedDescription = linkify(cleanedDescription);
    ...
    // --- تعديل جوهري هنا ---
    // تم حذف الكود الذي يضيف النص المساعد، لأنه أصبح مدمجًا في دالة linkify
...
function removeExtraLineBreaks(htmlContent) {
    if (!htmlContent) return '';
    const emptyParagraphRegex = /(<p><br><\/p>)+/g;
    return htmlContent.replace(emptyParagraphRegex, '');
}
