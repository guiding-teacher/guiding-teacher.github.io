...
let remoteShuffleTimers = { p1:null, p2:null, p3:null, p4:null };
let remoteShuffleSafety = { p1:null, p2:null, p3:null, p4:null };
...
const activeStrips = { p1:null, p2:null, p3:null, p4:null };
let lastMessageId = 0;
...
function celebrateLevelUp(newLevel){
  ...
  const anchor = inGame
    ? document.getElementById('panel'+String(session.role).toUpperCase())
...
function maybeAwardGameXP(room){
  if(!ALL_ROLES.includes(session.role)) return; // لا XP ولا إنجازات للمشاهد
  ...
  let opponentId, isWin;
  if(room.mode==='team'){
    const oppRoles = ludoTeamOf(session.role)==='A' ? ['p2','p4'] : ['p1','p3'];
    opponentId = room[oppRoles[0]+'_user_id'] || room[oppRoles[1]+'_user_id'];
    isWin = room.winner_team === ludoTeamOf(session.role);
  } else {
    opponentId = session.role==='p1' ? room.p2_user_id : room.p1_user_id;
    isWin = room.winner === session.role;
  }
...
function showNextAchievementToast(){
  ...
  burstReaction(document.getElementById(ALL_ROLES.includes(session.role) ? 'panel'+session.role.toUpperCase() : 'miniAvatar') || document.body, '🏅');
...
/* ===================== وضع الفريق (2×2) — دوال مساعدة إضافية بالكامل =====================
   لا تُستخدم أو تؤثر على اللعب الفردي إطلاقًا؛ فقط عندما room.mode==='team'.
   الفريق A = {p1,p3} (لونان متقابلان قطريًا: RED/ORANGE)، الفريق B = {p2,p4} (BLUE/GREEN).
   الدور يدور: p1 → p2 → p3 → p4 → p1 (يتبادل بين الفريقين في كل مرة). ====== */
const ALL_ROLES = ['p1','p2','p3','p4'];
const TEAM_COLORS = { p1:'RED', p2:'BLUE', p3:'ORANGE', p4:'GREEN' };
function ludoTeamOf(role){ return (role==='p1'||role==='p3') ? 'A' : (role==='p2'||role==='p4') ? 'B' : null; }
function ludoTeammateOf(role){ return { p1:'p3', p3:'p1', p2:'p4', p4:'p2' }[role] || null; }
function ludoNextRoleTeam(role){ return { p1:'p2', p2:'p3', p3:'p4', p4:'p1' }[role] || null; }
function activeRoles(room){ return (room && room.mode==='team') ? ALL_ROLES : ['p1','p2']; }
...
const PLAYABLE_COLORS = ['RED','ORANGE']; // لاعبان متقابلان الآن (الوضع الفردي) — وضع الفريق يستخدم TEAM_COLORS أعلاه
...
const ludoRemoteAnimating = { p1:null, p2:null, p3:null, p4:null }; // null أو رقم فهرس العروسة قيد التحريك الآن
...
function renderTokens(room, skip){
  ...
  const roleEntries = activeRoles(room).map(role=> [
    role,
    room[role+'_color'] || TEAM_COLORS[role] || 'RED',
    room[role+'_tokens'] || [0,0,0,0],
...
/* ===================== وضع الفريق (2×2) — إنشاء غرفة فريق والانضمام إليها =====================
   دوال إضافية بالكامل؛ لا تستبدل createRoom/joinRoomByCode الفرديتين، بل تُستدعى فقط من
   تبويب "فريق" الجديد (لإنشاء الغرفة)، بينما joinRoomByCode أدناه صار "ذكيًا": يكتشف تلقائيًا
   إن كان الرمز المُدخل يخص غرفة فريق فيحوّل الانضمام لمنطق الفريق، وإلا يكمل بمنطق الفردي كالمعتاد. ====== */
async function createTeamRoom(explicitCode){
  let code=explicitCode, ok=false, attempts=0;
  while(!ok && attempts<5){
    if(!code || attempts>0) code = randCode();
    const { error } = await sb.from('ludo_rooms').insert({
      code, status:'waiting', turn:'p1', mode:'team',
      p1_user_id:myId, p1_name:profile.username, p1_avatar_color:profile.avatar_color, p1_avatar_data:profile.avatar_data,
      p1_level: profile.level || 1, p1_title_ar: profile.title_ar || null, p1_title_icon: profile.title_icon || null,
      p1_color: TEAM_COLORS.p1, p2_color: TEAM_COLORS.p2, p3_color: TEAM_COLORS.p3, p4_color: TEAM_COLORS.p4,
      p1_tokens:[0,0,0,0], p2_tokens:[0,0,0,0], p3_tokens:[0,0,0,0], p4_tokens:[0,0,0,0],
      log:[], rev:0
    });
    if(!error) ok=true; attempts++;
  }
  if(!ok) return { error:'تعذّر إنشاء الفريق' };
  session.code=code; session.role='p1';
  saveSession();
  resetRoundXPTracking(); resetRoundKeys();
  subscribeToRoom(code); subscribeToPresence(code);
...
/* ترتيب امتلاء الفتحات في غرفة فريق: p1 (المُنشئ) ثم p3 (يكمل الفريق A)،
   ثم p2 (أول خصم) ثم p4 (يكمل الفريق B). عند اكتمال الأربعة تبدأ الجولة status='playing'. */
async function joinTeamRoomByCode(room, code){
  const mine = ALL_ROLES.find(r => room[r+'_user_id'] === myId);
  if(mine){
    session.code=code; session.role=mine; saveSession();
    resetRoundXPTracking(); resetRoundKeys();
    subscribeToRoom(code); subscribeToPresence(code);
    return { room };
  }

  const joinOrder = ['p3','p2','p4'];
  for(const role of joinOrder){
    if(room[role+'_user_id']) continue; // فتحة مشغولة بالفعل — جرّب التالية
    const patch = {
      [role+'_user_id']: myId, [role+'_name']: profile.username,
      [role+'_avatar_color']: profile.avatar_color, [role+'_avatar_data']: profile.avatar_data,
      [role+'_level']: profile.level || 1, [role+'_title_ar']: profile.title_ar || null, [role+'_title_icon']: profile.title_icon || null
    };
    if(role==='p4') patch.status = 'playing'; // آخر فتحة تُكمل الفريقين فتبدأ المباراة
    const { data: saved, error: err2 } = await sb.from('ludo_rooms').update(patch).eq('code', code).is(role+'_user_id', null).select().single();
    if(!err2 && saved){
      session.code=code; session.role=role; saveSession();
      resetRoundXPTracking(); resetRoundKeys();
      subscribeToRoom(code); subscribeToPresence(code);
      return { room: saved };
    }
    // فشل التحديث (سبقني أحد لنفس الفتحة تحديدًا) — أعد قراءة الغرفة وجرّب الفتحة التالية المتاحة
    const { data: latest } = await sb.from('ludo_rooms').select('*').eq('code', code).single();
    if(latest) room = latest;
  }

  // كل الفتحات الأربع ممتلئة ولستُ أحدها — أدخل كمشاهد
...
async function joinRoomByCode(code){
  ...
  if(room.mode === 'team') return joinTeamRoomByCode(room, code);
...
async function rollDice(forRole, isAuto=false){
  ...
  if(!ALL_ROLES.includes(actingRole)) return;
  ...
  const isTeamRoom = currentRoom.mode === 'team';

  clearTurnTimer(); clearWatchdogTimer();
  animating = true;
  if(isSelf){
    const b = document.getElementById('btnRoll'+actingRole.toUpperCase());
  ...
  if(result.roll_again){
    ...
    renderRoom(room);
    if(isSelf){
      const btn = document.getElementById('btnRoll'+actingRole.toUpperCase());
...
async function chooseToken(tokenIndex){
  ...
  const isSelf = actingRole === session.role;
  const isTeamRoom = currentRoom.mode === 'team';
  const myColor = currentRoom[actingRole+'_color'] || TEAM_COLORS[actingRole];
  animating = true;

  const { data, error } = await sb.rpc(isTeamRoom ? 'ludo_move_token_team' : 'ludo_move_token', {
...
function syncTurnControls(room, isSpectatorArg){
  if(room && room.mode==='team') return syncTurnControlsTeam(room, isSpectatorArg);
...
/* ====== نسخة وضع الفريق من ضبط أزرار الدور: يظهر زر الرمي فقط لصاحب الدور الحقيقي من الأربعة،
   وباقي الثلاثة (زميلي + الفريق الخصم بأكمله) يرون تسمية "آخر نتيجة" بدل الزر ====== */
function syncTurnControlsTeam(room, isSpectatorArg){
  const isSpectator = isSpectatorArg !== undefined ? isSpectatorArg : (session.role === 'spectator');
  ALL_ROLES.forEach(role=>{
    const btn = document.getElementById('btnRoll'+role.toUpperCase());
    const label = document.getElementById('oppLabel'+role.toUpperCase());
    if(!btn || !label) return;
    if(!isSpectator && role === session.role){
      btn.style.display = 'inline-flex';
      label.style.display = 'none';
      btn.disabled = !(room.status==='playing' && room.turn===session.role) || animating || !!room.awaiting_move;
    } else {
...
function renderRoom(room, opts={}){
  ...
  if(room.status==='waiting'){
    bannerText = room.mode==='team' ? '⏳ بانتظار اكتمال الفريقين (٢ ضد ٢)…' : '⏳ بانتظار انضمام اللاعب الثاني…';
  } else if(room.status==='finished'){
    if(room.mode==='team'){
      const teamLabel = room.winner_team==='A' ? 'أ' : 'ب';
      const finisherName = room.winner ? room[room.winner+'_name'] : null;
      bannerText = finisherName
        ? ('🏁 انتهت الجولة — فاز الفريق ' + teamLabel + ' (' + finisherName + ')')
        : ('🏁 انتهت الجولة — فاز الفريق ' + teamLabel);
    } else {
      const winnerName = room.winner==='p1' ? room.p1_name : room.p2_name;
      bannerText = '🏁 انتهت الجولة — الفائز: ' + winnerName;
    }
  } else {
    const turnName = room[room.turn+'_name'];
    if(isSpectator){
  ...
  renderTeamExtras(room, isSpectator);
...
/* ====== يملأ بطاقتي زميلي الفريق (p3/p4) ويضبط تسميات/كلاسات وضع الفريق — إضافي بالكامل،
   يُستدعى من renderRoom دون أن يغيّر أي شيء فيها؛ في الوضع الفردي يُخفي كل شيء فورًا ====== */
function renderTeamExtras(room, isSpectator){
  ...
  ['p3','p4'].forEach(role=>{
    ...
    const name = room[role+'_name'];
    const nameEl = document.getElementById('name'+R);
    if(nameEl) nameEl.textContent = name || 'بانتظار انضمام…';
    const posEl = document.getElementById('pos'+R);
    if(posEl) posEl.textContent = tokensSummaryText(room[role+'_tokens']);
    applyAvatarVisual(document.getElementById('avatar'+R), room[role+'_avatar_color'], room[role+'_avatar_data'], name?name[0]:'?');
    renderLevelBadge(document.getElementById('levelBadge'+R), room[role+'_level'], true);
    renderLevelBadge(document.getElementById('nameLevel'+R), room[role+'_level'], false);
    renderTitleBadge(document.getElementById('nameTitle'+R), room[role+'_title_ar'], room[role+'_title_icon']);
    const panel = document.getElementById('panel'+R);
    if(panel){
      panel.classList.toggle('active-turn', room.turn===role && room.status==='playing');
      panel.classList.toggle('me', session.role===role);
      panel.classList.toggle('opponent', session.role!==role);
    }
    const roleTagEl = document.getElementById('roleTag'+R);
    if(roleTagEl) roleTagEl.textContent = playerRoleLabel(room, role, isSpectator);
    const editBadgeEl = document.getElementById('editBadge'+R);
    if(editBadgeEl) editBadgeEl.style.display = session.role===role ? 'flex':'none';
    showDiceValue(role, room[role+'_dice']||1, false);
  });
}
...
function renderPerPlayerLogs(room){
  ...
  const log = room.log || [];
  const names = {}; roles.forEach(r=> names[r] = room[r+'_name'] || '');
  const lines = {}; roles.forEach(r=> lines[r] = []);
  log.forEach(line=>{
    for(const r of roles){
      if(names[r] && line.includes(names[r])){ lines[r].push(line); break; }
    }
  });
  const defaultNames = { p1:'اللاعب الأول', p2:'اللاعب الثاني', p3:'اللاعب الثالث', p4:'اللاعب الرابع' };
  roles.forEach(r=>{
    const box = document.getElementById('logBox'+r.toUpperCase());
    if(box){
      box.innerHTML = lines[r].length
        ? lines[r].slice(-6).reverse().map(l=>`<div>${l}</div>`).join('')
        : '<div class="empty">لا أحداث بعد</div>';
    }
    fullPlayerLogs[r] = lines[r];
    fullPlayerNames[r] = names[r] || defaultNames[r];
  });
  if(openEventsRole) renderEventsSheetBody(openEventsRole);
}

/* ====== سلايد أحداث اللاعب — تصميم الهاتف فقط (يفتح بدل زر القلم) ====== */
const fullPlayerLogs = { p1:[], p2:[], p3:[], p4:[] };
const fullPlayerNames = { p1:'', p2:'', p3:'', p4:'' };
...
document.getElementById('logBadgeP3')?.addEventListener('click', ()=> openEventsSheet('p3'));
document.getElementById('logBadgeP4')?.addEventListener('click', ()=> openEventsSheet('p4'));
...
function clearTurnTimer(){
  ...
  if(ALL_ROLES.includes(session.role)){
    const btn = document.getElementById('btnRoll'+session.role.toUpperCase());
...
function armTurnTimer(){
  ...
  const btn = document.getElementById('btnRoll'+String(session.role).toUpperCase());
...
function openWinModal(room){
  ...
  if(room.mode==='team') return openWinModalTeam(room);
  const isSpectator = session.role === 'spectator';
...
/* ====== نافذة فوز وضع الفريق: يظهر أولًا من أنهى عرائسه الأربع، وبجانبه صورة/اسم زميله
   بالفريق — تحقيقًا لطلب "الذي أدخل جميع عرائسه يصبح الأول بالتسلسل والثاني صديقه بالفريق" ====== */
function openWinModalTeam(room){
  const modal = document.getElementById('winModal');
  const isSpectator = session.role === 'spectator';
  const myTeam = ludoTeamOf(session.role);
  const isMyTeamWin = !!myTeam && myTeam === room.winner_team;
  const teamLabel = room.winner_team==='A' ? 'أ' : 'ب';

  document.getElementById('winTitle').textContent = isMyTeamWin ? '🎉 فريقك فاز!' : ('فاز الفريق ' + teamLabel);

  const teamRow = document.getElementById('winTeamRow');
  if(room.winner && teamRow){
    const finisherRole = room.winner;
    const mateRole = ludoTeammateOf(finisherRole);
    document.getElementById('winText').textContent =
      'أدخل ' + (room[finisherRole+'_name']||'أحد أفراد الفريق') + ' جميع عرائسه إلى البيت أولًا، ففاز الفريق بالكامل!';
    document.getElementById('winFinisherName').textContent = room[finisherRole+'_name'] || '—';
    document.getElementById('winMateName').textContent = room[mateRole+'_name'] || '—';
    applyAvatarVisual(document.getElementById('winFinisherAvatar'), room[finisherRole+'_avatar_color'], room[finisherRole+'_avatar_data'], (room[finisherRole+'_name']||'?')[0]);
    applyAvatarVisual(document.getElementById('winMateAvatar'), room[mateRole+'_avatar_color'], room[mateRole+'_avatar_data'], (room[mateRole+'_name']||'?')[0]);
    teamRow.style.display = 'flex';
  } else {
    document.getElementById('winText').textContent = 'انتهت الجولة لصالح الفريق ' + teamLabel + '.';
    if(teamRow) teamRow.style.display = 'none';
  }

  document.getElementById('btnRematch').style.display = 'none'; // إعادة مباراة فريق فورية غير مدعومة بعد — يمكن إنشاء فريق جديد من الرئيسية
  modal.style.display='flex';
  launchConfetti();
  beep(880,.2,'triangle'); setTimeout(()=>beep(1100,.25,'triangle'),150);

  if(!isSpectator && myTeam){
    const oppRoles = myTeam==='A' ? ['p2','p4'] : ['p1','p3'];
    const oppNames = oppRoles.map(r=> room[r+'_name']).filter(Boolean).join(' و ');
    const roundKey = room.code + '|' + room.rev;
    if(historySavedRoundKey !== roundKey){
      historySavedRoundKey = roundKey;
      writeRoundFlag('ludo_historySavedRoundKey', roundKey);
      saveHistoryEntry({
        date: new Date().toLocaleString('ar', {dateStyle:'medium', timeStyle:'short'}),
        opponent: (oppNames || 'فريق خصم') + ' (فريق)',
        result: isMyTeamWin ? 'win' : 'lose'
      });
    }
  }
}
...
function fireReaction(role, emoji){
  const anchor = document.getElementById('panel'+String(role).toUpperCase());
  if(anchor) burstReaction(anchor, emoji);
}
function broadcastReaction(emoji){ presenceChannel?.send({ type:'broadcast', event:'react', payload:{emoji, from:session.role} }); }
async function sendChatMessage(text){
  if(!ALL_ROLES.includes(session.role)) return; // المشاهد لا يرسل رسائل
  const name = currentRoom ? currentRoom[session.role+'_name'] : null;
...
function mergeRoomPayload(incoming, prev){
  ...
  ['p1_name','p2_name','p3_name','p4_name','p1_avatar_color','p2_avatar_color','p3_avatar_color','p4_avatar_color','p1_avatar_data','p2_avatar_data','p3_avatar_data','p4_avatar_data'].forEach(key=>{
...
function subscribeToRoom(code){
  ...
  window._roomPoll = setInterval(async ()=>{
    ...
    try{
      ...
      if(room && !animating && ALL_ROLES.every(r=> ludoRemoteAnimating[r]==null) && (!currentRoom || currentRoom.rev !== room.rev || currentRoom.status !== room.status)){
        renderRoom(room);
      }
    }catch(e){}
    pollMissedMessages();
...
async function refreshRoomNow(){
  ...
  try{
    const { data:room } = await sb.from('ludo_rooms').select('*').eq('code', session.code).single();
    if(room && !animating && ALL_ROLES.every(r=> ludoRemoteAnimating[r]==null)) renderRoom(room);
  }catch(e){}
  pollMissedMessages();
}
...
function subscribeToPresence(code){
  ...
  presenceChannel
    .on('presence', {event:'sync'}, ()=>{
      ...
      ALL_ROLES.forEach(r=>{
        const liveEl = document.getElementById('live'+r.toUpperCase());
        if(liveEl) liveEl.style.display = state[r] ? 'block':'none';
      });
...
function leaveRoom(){
  ...
  clearWatchdogTimer();
  ALL_ROLES.forEach(r=>{
    clearInterval(remoteShuffleTimers[r]); clearTimeout(remoteShuffleSafety[r]);
    remoteShuffleTimers[r] = null; remoteShuffleSafety[r] = null;
    ludoRemoteAnimating[r] = null;
  });
  cancelPendingRender();
...
async function resumeSavedSession(code, fallbackLinkCode){
  try{
    const { data:room } = await sb.from('ludo_rooms').select('*').eq('code', code).single();
    const myRole = room ? (ALL_ROLES.find(r => room[r+'_user_id'] === myId) || null) : null;
    const savedInfo = loadSession();
    if(room && room.status !== 'finished' && (myRole || (savedInfo && savedInfo.role==='spectator'))){
      session.code = code;
      session.role = myRole || 'spectator';
      saveSession();
      if(myRole){ resetRoundXPTracking(); resetRoundKeys(); }
...
function initReactionButtons(){
  ...
  document.querySelectorAll('.side-reactions button').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      const panelId = btn.closest('.side-panel').id; // 'panelP1'..'panelP4'
      const role = panelId.replace('panel','').toLowerCase();
...
document.getElementById('editBadgeP3')?.addEventListener('click', ()=>{ if(session.role==='p3') openEditProfile(); });
document.getElementById('editBadgeP4')?.addEventListener('click', ()=>{ if(session.role==='p4') openEditProfile(); });
...
document.getElementById('btnCreateTeam')?.addEventListener('click', async ()=>{
  const { code, room, error } = await createTeamRoom();
  if(error){ alert(error); return; }
  await loadChatHistory(code);
  enterGameScreen(room);
});
...
document.getElementById('btnRollP3')?.addEventListener('click', ()=>{ if(session.role==='p3') rollDice('p3', false); });
document.getElementById('btnRollP4')?.addEventListener('click', ()=>{ if(session.role==='p4') rollDice('p4', false); });
...
async function handleLeaveAsLoss(){
  if(!ALL_ROLES.includes(session.role)) return;
  if(session.code && currentRoom && currentRoom.status==='playing' && session.role){
    const isTeamRoom = currentRoom.mode === 'team';
    let oppName;
    if(isTeamRoom){
      const oppRoles = ludoTeamOf(session.role)==='A' ? ['p2','p4'] : ['p1','p3'];
      oppName = oppRoles.map(r=> currentRoom[r+'_name']).filter(Boolean).join(' و ');
    } else {
      oppName = session.role==='p1' ? currentRoom.p2_name : currentRoom.p1_name;
    }
    ...
    try{
      await sb.rpc(isTeamRoom ? 'ludo_leave_as_loss_team' : 'ludo_leave_as_loss', { p_code: session.code, p_role: session.role });
    }catch(e){}
    saveHistoryEntry({
      date: new Date().toLocaleString('ar', {dateStyle:'medium', timeStyle:'short'}),
      opponent: (oppName || 'خصم') + (isTeamRoom ? ' (فريق)' : ''),
      result: 'lose'
    });
...
async function endCurrentSessionAsLeave(code){
  try{
    const { data: room } = await sb.from('ludo_rooms').select('*').eq('code', code).single();
    if(!room) return;
    const myRole = ALL_ROLES.find(r => room[r+'_user_id'] === myId) || null;
    if(!myRole) return; // لم يكن لاعبًا فيها (كان مشاهدًا مثلًا) — لا حاجة لأي إجراء
    const isTeamRoom = room.mode === 'team';
    if(room.status==='playing'){
      let oppName;
      if(isTeamRoom){
        const oppRoles = ludoTeamOf(myRole)==='A' ? ['p2','p4'] : ['p1','p3'];
        oppName = oppRoles.map(r=> room[r+'_name']).filter(Boolean).join(' و ');
      } else {
        oppName = myRole==='p1' ? room.p2_name : room.p1_name;
      }
      try{
        await sb.rpc(isTeamRoom ? 'ludo_leave_as_loss_team' : 'ludo_leave_as_loss', { p_code: code, p_role: myRole });
      }catch(e){}
      saveHistoryEntry({
        date: new Date().toLocaleString('ar', {dateStyle:'medium', timeStyle:'short'}),
        opponent: (oppName || 'خصم') + (isTeamRoom ? ' (فريق)' : ''),
